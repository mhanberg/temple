defmodule Temple.Ast.Utils do
  ...
  @attributes_escaper Application.compile_env(
                        :temple,
                        :attributes_escape,
  ...
  def compile_attrs(attrs) when is_list(attrs) do
    if Keyword.keyword?(attrs) do
      ...
      [
        {:expr,
         quote do
           unquote(__MODULE__).__attributes_escape__(unquote(List.first(attrs)))
         end}
      ]
    end
  end
  ...
  def build_attr("rest!", {_, _, _} = value) do
    expr =
      quote do
        unquote(__MODULE__).__attributes_escape__(unquote(value))
      end
  ...
  def build_attr(name, {_, _, _} = value) do
    expr =
      quote do
        unquote(__MODULE__).__attributes_escape__([{unquote(name), unquote(value)}])
      end
  ...
  def __attributes_escape__(attributes) do
    {mod, func} = @attributes_escaper
